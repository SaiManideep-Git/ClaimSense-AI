...
const createCanvasDocument = (
  ...
  return new Promise((resolve) => {
    ...
    if (type === 'prescription') {
      ...
      ctx.fillText(`DATE: ${input.treatment_date || '2024-11-01'}`, 400, 145);
      ...
      if (pres.medicines_prescribed) {
        pres.medicines_prescribed.forEach((med: string) => {
      ...
      if (pres.procedures) {
        pres.procedures.forEach((proc: string) => {
      ...
      if (pres.tests_prescribed) {
        ...
        ctx.font = '13px Courier New';
        ctx.fillStyle = '#475569';
        pres.tests_prescribed.forEach((test: string) => {
      ...
      ctx.fillText(`Date of Test :  ${input.treatment_date || '2024-11-01'}`, 380, 145);
      ...
      Object.keys(bill).forEach((key) => {
        // Skip summary fields in the main line items list
        if (summaryKeys.includes(key.toLowerCase())) {
          return;
        }

        // Avoid double-counting diagnostic_tests total if test_names are detailed
        ...
        if (typeof val === 'number') {
          ...
          // If this is an array like test_names, we split the parent category cost (diagnostic_tests) equally
